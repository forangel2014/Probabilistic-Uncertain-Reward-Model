...
class PPOTrainer(ABC):
    ...
    def __init__(
        ...
        custom_reward_model: Callable = None,
        **generate_kwargs,
    ) -> None:
        assert (
    ...
    def fit(
        ...
        print(f"{consumed_samples=}")
        print(f"{args.train_batch_size=}")
        print(f"{args.max_epochs=}")
        print(f"{args.rollout_batch_size=}")
        print(f"{args.n_samples_per_prompt=}")
        print(f"{num_rollouts_per_episodes=}")
        ...
        for episode in range(start_episode, args.num_episodes):
            ...
            for batch_data in self.prompts_dataloader:
                for i, experience in enumerate(
                    self.experience_maker.make_experience_list(batch_data, steps, **self.generate_kwargs)
                ):
                    if i == 0:
