...
class PGRMTrainer(RewardTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        for i in range(5):
            mean_chosen = logits_chosen[:, i]
            mean_rejected = logits_rejected[:, i]
            log_var_chosen = logits_chosen[:, i+5]
            log_var_rejected = logits_rejected[:, i+5]

            temperature = 10.0
            var_chosen = torch.exp(log_var_chosen/temperature)
            var_rejected = torch.exp(log_var_rejected/temperature)

            r_chosen = torch.randn(1).to(var_chosen.device).to(torch.float16) * var_chosen + mean_chosen
            r_rejected = torch.randn(1).to(var_rejected.device).to(torch.float16) * var_rejected + mean_rejected

            reward_server = "http://localhost:10377"
            xdata = make_bytes_list([json.dumps({}).encode(), tensor_to_bytes(inputs["input_ids_chosen"].to("cpu"))])
            response = requests.post(f"{reward_server}/upload", data=xdata).content
            dd = bytes_list_to_list(response)
            reward_chosen = bytes_to_tensor(dd[2])

            xdata = make_bytes_list([json.dumps({}).encode(), tensor_to_bytes(inputs["input_ids_rejected"].to("cpu"))])
            response = requests.post(f"{reward_server}/upload", data=xdata).content
            dd = bytes_list_to_list(response)
            reward_rejected = bytes_to_tensor(dd[2])

            p_pos = torch.sigmoid(reward_chosen - reward_rejected).to(r_chosen.device)
            p_neg = 1 - p_pos

            #print(r_chosen, r_rejected, p_pos, p_neg)

            alpha = 10
            loss += (r_chosen - alpha*(p_pos.detach() - 0.5)).pow(2).mean() + (r_rejected - alpha*(p_neg.detach() - 0.5)).pow(2).mean()
